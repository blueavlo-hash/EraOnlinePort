...
func _load_all_data() -> void:
	...
	# Load combat abilities (string keys → int-keyed internal dict)
	var raw_abilities := _load_json("combat_abilities.json")
	_ability_data.clear()
	for key in raw_abilities:
		_ability_data[int(key)] = raw_abilities[key]

	is_loaded = true
	var elapsed := Time.get_ticks_msec() - start
	print("[GameData] Loaded in %dms | GRH:%d OBJ:%d NPC:%d SPL:%d ABL:%d Maps:%d" % [
		elapsed, grh_data.size(), objects.size(), npcs.size(), spells.size(), _ability_data.size(), map_index.size()
	])
