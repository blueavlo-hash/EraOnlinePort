...
def parse_npcs(
    ...
    if os.path.exists(src2):
        ini2 = INIParser(src2)
        # NPC2 entries stored at index 500+; the file may use NPC500, NPC501, etc.
        # or it may restart numbering at NPC1. We try NPC1 first and remap.
        count2 = ini2.get_int("INIT", "NumNPCs", 0)
        if count2 == 0:
            count2 = 500
        npc2_entries: dict[str, dict] = {}
        n2 = _parse_file(ini2, npc2_entries, "NPC", count2)
        # Remap indices: if they start at 1, shift to 500+
        for k, v in npc2_entries.items():
            new_key = str(int(k) + 499) if int(k) < 500 else k
            entries[new_key] = v
        print(f"[parse_npcs] Parsed {n2} NPCs from NPC2.dat")
        total += n2
    else:
